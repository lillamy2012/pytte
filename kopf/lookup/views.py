...
from lookup.models import Annotation, Project, Scientist, ProjectBlogg, CommentForm, Stats, Antibody, AntibodyForm, DeleteABForm, KitForm, Kit, ProtocolDocForm, Protocol, Seed, SeedRelation, SeedForm,  Seed1stForm, ContactForm, SeedContact, Multiplex
...
def allsamples(request):
    type = request.GET.get('type')
    scientist = request.GET.get('person')
    if scientist == None:
        ...
        return render(request, 'lookup/allsamples.html',{'RNAanno': RNA,'ChipAnno': CHIP, 'type' : type, 'person' : scientist})
    else:
        RNA = serializers.serialize("python", Annotation.objects.filter(exptype="RNA-Seq",scientist=scientist),fields=('sample','scientist','genotype'))
        CHIP = serializers.serialize("python", Annotation.objects.filter(exptype="ChIP-Seq",scientist=scientist),fields=('sample','scientist','antibody','genotype'))
        return render(request, 'lookup/allsamples.html',{'RNAanno': RNA,'ChipAnno': CHIP, 'type' : type, 'person' : scientist})
